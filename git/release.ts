...
const bumpType: BumpType | undefined = shouldMajor ? 'major' 
   : shouldMinor ? 'minor' 
   : shouldPatch ? 'patch' 
   : undefined;
...
async function handleRelease() {
   ...
   const rollback = async () => {
      ...
      if (await backupFile.exists()) {
         ...
         await Bun.write("package.json", JSON.stringify(currentPackageJson, null, 3));
   ...
   if (exitCode === 0) {
      ...
      const result = await $`gh release create ${tagName} vbl-pro-bun.zip --generate-notes`;
